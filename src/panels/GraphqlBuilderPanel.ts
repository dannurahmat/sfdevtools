...
export class GraphqlBuilderPanel {
    public static currentPanel: GraphqlBuilderPanel | undefined;
    ...
    public static createOrShow(extensionUri: vscode.Uri) {
        ...
        if (GraphqlBuilderPanel.currentPanel) {
            GraphqlBuilderPanel.currentPanel._panel.reveal(column);
            return;
        }

        const panel = vscode.window.createWebviewPanel(
            'sfDevToolsGraphqlBuilder',
            'GraphQL Explorer',
        ...
        GraphqlBuilderPanel.currentPanel = new GraphqlBuilderPanel(panel, extensionUri);
    }
...
<head>
    ...
    <style>
        ...
        .subquery-cell { padding: 8px 14px; }
...
<body>
    ...
    <div class="main">
        <div class="tabs">
            ...
            <div class="tab" id="json-tab-header" onclick="switchTab('json')">JSON Result</div>
        </div>
        ...
        <div id="tab-builder" class="tab-content active">
            <div class="builder-container">
                <div class="selectors-lat" id="selectors-container">
                    ...
                    <div class="selector-section" id="child-rel-section">
                        <div class="fields-header">
                            <div class="fields-title-row">
                ...
                <div class="query-lat">
                    <div class="query-box-header">GraphQL Query</div>
                    <textarea id="graphql-query" class="query-box" placeholder="query { uiapi { query { Account(first: 10) { edges { node { Name { value } } } } } } }"></textarea>
        ...
        <div id="tab-json" class="tab-content">
             <div style="display:flex; justify-content:space-between; align-items:center; padding: 10px 20px; border-bottom: 1px solid var(--vscode-panel-border); background-color: var(--vscode-sideBar-background);">
                <span style="font-size: 11px; text-transform: uppercase;">Raw JSON Response</span>
                <button class="btn-small secondary" onclick="copyJsonResult()">Copy JSON</button>
            </div>
            <textarea id="json-result-box" readonly></textarea>
        </div>
    </div>
    ...
    <script nonce="${nonce}">
        ...
        window.addEventListener('message', event => {
            ...
            switch(message.command) {
                ...
                case 'setResults':
                    ...
                    document.getElementById('json-result-box').value = JSON.stringify(message.result, null, 2);
        ...
        function updateQuery() {
            ...
            const buildQueryBody = (node, pathContext = '') => {
                ...
                return keys.map(key => {
                    ...
                    if (!pathContext) {
                        isChildRel = baseChildRels && baseChildRels.some(r => r.relationshipName === key);
                    } else {
                        if (relData[pathContext] && relData[pathContext].childRelationships) {
                            isChildRel = relData[pathContext].childRelationships.some(r => r.relationshipName === key);
                        }
                    }
                    ...
                    if (node[key] === true) {
                        ...
                        return \`\${key} { value }\`;
                         ...
                         const inner = buildQueryBody(node[key], currentPath);
                         if (isChildRel) {
                             return \`\${key}(first: 5) { edges { node { \${inner} } } }\`;
                         } else {
                             return \`\${key} { \${inner} }\`;
                         }
                    }
            ...
            const fieldsBlock = buildQueryBody(root, '');
            
            const q = \`query {
  uiapi {
    query {
      \${currentObject}(first: 50) {
        edges {
          node {
            \${fieldsBlock}
          }
            ...
            document.getElementById('graphql-query').value = q;
        }
        ...
        function copyJsonResult() {
            const q = document.getElementById('json-result-box').value;
            if(!q) return;
            const el = document.createElement('textarea'); el.value = q; document.body.appendChild(el);
            el.select(); document.execCommand('copy'); document.body.removeChild(el);
        ...
        function renderResults(result) {
            ...
            try {
                if (result.errors && result.errors.length > 0) {
                     list.innerHTML = \`<div style="padding:20px; color:red;">GraphQL Errors:<br><pre>\${JSON.stringify(result.errors, null, 2)}</pre></div>\`;
                     return;
                }
                
                const data = result.data.uiapi.query;
                const objKey = Object.keys(data)[0];
                const edges = data[objKey].edges;
                    ...
                    Object.keys(node).forEach(k => {
                        ...
                        if (val && typeof val === 'object') {
                            if ('value' in val) {
                                ...
                                // Subquery result
                                flat[key + ' (Subquery)'] = val.edges.map(e => flattenNode(e.node)); 
                            } else {
                                const sub = flattenNode(val, key);
                                flat = { ...flat, ...sub };
                            }
                        } else {
                            flat[key] = val;
                ...
                const records = edges.map(e => flattenNode(e.node));
                ...
                cols.forEach(c => {
                    ...
                    th.textContent = c.replace(' (Subquery)', '');
                ...
                records.forEach(r => {
                    ...
                    cols.forEach(c => {
                         ...
                         if (c.endsWith(' (Subquery)')) {
                            ...
                            const subRecords = val; 
                            
                            td.className = 'subquery-cell';
                            
                            const info = document.createElement('span');
                            info.className = 'subquery-count';
                            info.textContent = \`\${subRecords ? subRecords.length : 0} records\`;
                            td.appendChild(info);
                            ...
                            if (subRecords && subRecords.length > 0) {
                                ...
                                jsonBtn.onclick = () => vscode.postMessage({ 
                                    command: 'saveFile', 
                                    content: JSON.stringify(subRecords, null, 2), 
                                    fileName: \`\${relName}_graphql_export.json\` 
                                });
                         ...
                         } else if (c.endsWith('.Id') || c === 'Id') {
                             if (val) {
                                td.innerHTML = \`<span class="id-link">\${val}</span>\`;
                                td.addEventListener('click', (e) => {
                                    e.stopPropagation(); 
                                    const type = currentObject; 
                                    showContextMenu(e.clientX, e.clientY, val, null, type);
                                });
                             }
                         } else {
                ...
                list.innerHTML = \`<div style="padding:20px; color:red;">Error parsing results: \${e.message}<br><pre>\${JSON.stringify(result, null, 2)}</pre></div>\`;
            }
        ...
        function switchTab(tabName) {
            ...
            const idx = tabName === 'builder' ? 0 : (tabName === 'results' ? 1 : 2);
...
function isDisposable(x: any): x is vscode.Disposable {
